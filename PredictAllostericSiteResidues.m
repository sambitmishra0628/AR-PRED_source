function [] = PredictAllostericSiteResidues(outputdir, pdb_ca_file)
    ...
    model_wts = [0.4918 0.5287 0.4752 0.4827 0.5030 0.4854 0.5177 0.4843 0.4628 0.4599]'; % consider weights as MCCs of individual models
    ...
    model_path = strcat(arpred_path, '/', 'rf_models_allosteric_site');
    feature_matrix = ConsolidatedFeatureMatrix(outputdir);
    ...
    pred_output_file = strcat(outputdir, '/', 'allostericsite_predictions.csv');
...
function [feature_matrix] = ConsolidatedFeatureMatrix(outputdir)
    feature_file_names = {'sasa_features.csv', 'msf.csv', 'conservation.csv', 'ntw_cen_adj.csv', 'ntw_cen_dist_int.csv',...
        'ntw_cen_bt_allostericsite.csv', 'ntw_cen_corr_allostericsite.csv', 'pocket_features.csv', 'dfi_allostericsite.csv', 'shortest_path.csv'};
    feature_matrix = [];
